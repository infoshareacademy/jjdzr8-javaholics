...
public final class FileService {
    ...
    public Routes readRoutesFromFile() {
        ...
        if (Objects.nonNull(routes)) {
            System.out.println("Plik właściwie zaimportowany. Ilość tras: " + routes.getRoutes().size());
            return routes;
        } else {
            return new Routes();
        }
    ...
    private void writeObjectToJsonFile(Object objectToSave, Path pathToSave) {
    ...
    private <T> T readDataFromJsonFile(Class<T> dataType, Path path) {
        try (Reader reader = new FileReader(path.toFile())) {
            System.out.println("Zaczytuję plik: " + pathRoutesFile);
